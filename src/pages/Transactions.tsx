...
import {
  ...
  FileText, Table, Users, ChevronLeft, ChevronRight, Calendar, AlertTriangle
} from 'lucide-react';
...
export const Transactions: React.FC<TransactionsProps> = ({ transactions, onDelete }) => {
  ...
  // --- STATE UNTUK POP-UP MODAL KONFIRMASI HAPUS ---
  ...
  // Handle ketika tombol tong sampah diklik
  const triggerDeleteConfirm = (id: string) => {
    setSelectedTxId(id);
    setIsModalOpen(true);
  };

  // Handle ketika aksi hapus dikonfirmasi di dalam modal
  ...
  return (
    <div className="space-y-6 relative">
                ...
                {filteredTransactions.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-center">
                      {isOwner && (
                        <button
                          onClick={() => triggerDeleteConfirm(t.id)}
          ...
          {filteredTransactions.map((t) => (
            <div key={t.id} className="rustic-card p-4 flex items-center justify-between">
              ...
              <div className="text-right flex flex-col items-end space-y-2">
                ...
                {isOwner && (
                  <button onClick={() => triggerDeleteConfirm(t.id)} className="p-1 text-slate-300 hover:text-rose-500 transition-colors">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
      ...
      {/* --- POP-UP MODAL BOX DI SINI --- */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl border border-slate-200 p-6 max-w-sm w-full shadow-2xl space-y-4 animate-scale-up">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-sm font-black uppercase tracking-wider text-slate-900">Hapus Transaksi</h3>
                <p className="text-xs text-slate-500 mt-0.5">Tindakan ini tidak bisa dibatalkan. Apakah Anda yakin?</p>
              </div>
            </div>
            <div className="flex space-x-2 pt-2">
              <button
                ...
                className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider border border-slate-200 text-slate-500 rounded-xl hover:bg-slate-50 transition-colors"
              >
                Batal
              </button>
              <button
                onClick={handleConfirmDelete}
                className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider bg-rose-500 text-white rounded-xl hover:bg-rose-600 transition-colors shadow-sm active:scale-95"
              >
                Ya, Hapus
              </button>
            </div>
